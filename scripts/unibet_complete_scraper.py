"""
🔥 COMPLETE UNIBET SCRAPER 🔥
Scrapes ALLE voetbalwedstrijden met odds van Unibet via Kambi API

Features:
- Direct API access (geen browser!)
- Alle voetbal competities
- 1X2, Over/Under, BTTS odds
- Ready voor AI training
"""
...
class UnibetCompleteScraper:
    """Complete Unibet scraper via Kambi API"""
    
    def __init__(self):
        self.base_url = "https://eu1.offering-api.kambicdn.com/offering/v2018/ubnl"
        self.football_group_id = "1000093190"  # Voetbal group ID
    ...
    def fetch_all_football_matches(self):
        ...
        logger.info("🔍 Fetching all football matches...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raw_file = f'data/unibet_raw_{timestamp}.json'
    ...
    def save_matches(self, matches):
        ...
        # Save CSV
        csv_file = f'data/unibet_matches_{timestamp}.csv'
        df.to_csv(csv_file, index=False)
        logger.info(f"💾 CSV saved: {csv_file}")
        
        # Save JSON
        json_file = f'data/unibet_matches_{timestamp}.json'
    ...
    def run(self):
        """Main execution"""
        logger.info("="*80)
        logger.info("🔥 UNIBET COMPLETE SCRAPER")
        logger.info("="*80)
...
if __name__ == '__main__':
    print("="*80)
    print("🔥 UNIBET COMPLETE SCRAPER")
    print("Direct API - Fast & Reliable!")
    print("="*80)
    
    try:
        scraper = UnibetCompleteScraper()
        matches = scraper.run()
