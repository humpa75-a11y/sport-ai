#!/usr/bin/env python3
"""
🇳🇱 EREDIVISIE SPECIALIST! 🇳🇱
TRAIN ALLEEN OP EREDIVISIE DATA!

Waarom? Elke competitie heeft andere speelstijlen:
- Premier League: Defensief (veel 1-0, 0-0)
- Bundesliga: Offensief (veel 3-2, 4-1)
- Eredivisie: WILD (veel 2-2, 3-3, doelpunten!)

Gebruiker voorspelt NEDERLANDSE wedstrijden, dus train op NEDERLANDSE data!
"""
...
print("="*80)
print("🇳🇱 EREDIVISIE SPECIALIST - TRAIN OP NEDERLANDSE DATA! 🇳🇱")
print("="*80)
...
# FILTER ALLEEN EREDIVISIE!
matches = [m for m in all_matches if m.get('league') == 'Eredivisie']

print(f"✅ {len(all_matches)} totale wedstrijden geladen!")
print(f"✅ {len(matches)} Eredivisie wedstrijden gefilterd!")
print(f"📊 Percentage Eredivisie: {len(matches)/len(all_matches)*100:.1f}%")
...
print("\n📊 Training EREDIVISIE SPECIALIST models...")
...
print("\n🇳🇱 EREDIVISIE SPECIALIST RESULTATEN:")
...
model_path = 'data/de_meester_EREDIVISIE.pkl'
...
print("\n" + "="*80)
print("🇳🇱 EREDIVISIE SPECIALIST KLAAR! 🏆🏆")
print(f"   📊 {len(matches)} Eredivisie matches | {exact_pct:.2f}% exact")
print(f"   🎯 {result_pct:.2f}% result accuracy!")
print("="*80)
